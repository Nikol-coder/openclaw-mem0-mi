 ...
 *
 * Long-term memory via Mem0 — supports:
 * - Mem0 platform (mem0ai package)
 * - Open-source self-hosted SDK
 * - External API (custom HTTP endpoints)
 *
 * Features:
...
type Mem0Mode = "platform" | "open-source" | "external-api";
...
// ============================================================================
// External API Provider (调用外部 HTTP API)
// ============================================================================
...
class ExternalAPIProvider implements Mem0Provider {
  ...
  async add(
    ...
    try {
      ...
      throw new Error(`External API add failed: ${String(err)}`);
    }
  }
  ...
  async search(query: string, options: SearchOptions): Promise<MemoryItem[]> {
    ...
    try {
      ...
      const levelMapping: Record<string, string> = {
        ...
        "l1记忆": "l1",
        "l2记忆": "l2",
        session_memories_raw: "session",
        filtered_knowledge: "filtered",
      };

      for (const [key, level] of Object.entries(levelMapping)) {
        const content = data[key];
        if (content && typeof content === "string" && content.trim()) {
          memories.push({
            id: `ext-${level}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
            memory: content,
            user_id: options.user_id,
            score: 1.0,
            categories: [level],
      ...
      throw new Error(`External API search failed: ${String(err)}`);
    }
  }
  ...
  async get(memoryId: string): Promise<MemoryItem> {
    return {
      id: memoryId,
      memory: "Individual memory retrieval not supported by external API",
    };
  }
  ...
  async delete(memoryId: string): Promise<void> {
    console.warn("External API does not support delete operation");
  }
}
  ...
  parse(value: unknown): Mem0Config {
    ...
    if (mode === "platform") {
      if (typeof cfg.apiKey !== "string" || !cfg.apiKey) {
        throw new Error(
          'apiKey is required for platform mode (set mode: "open-source" or "external-api" for alternatives)',
        );
      }
    }

    // External API mode requires externalApi config
    if (mode === "external-api") {
      if (!cfg.externalApi || typeof cfg.externalApi !== "object") {
        throw new Error(
          "externalApi config is required for external-api mode",
        );
      }
      const extApi = cfg.externalApi as Record<string, unknown>;
      if (!extApi.searchUrl || !extApi.addUrl) {
        throw new Error(
          "externalApi.searchUrl and externalApi.addUrl are required",
        );
      }
    }
    ...
    // Resolve env vars in externalApi config
    let externalApiConfig: Mem0Config["externalApi"];
    if (
      cfg.externalApi &&
      typeof cfg.externalApi === "object" &&
      !Array.isArray(cfg.externalApi)
    ) {
      const extApi = resolveEnvVarsDeep(
        cfg.externalApi as Record<string, unknown>,
      );
      externalApiConfig = {
        searchUrl: extApi.searchUrl as string,
        addUrl: extApi.addUrl as string,
        apiToken: (extApi.apiToken as string) || "",
        timeout: (extApi.timeout as number) || 10000,
      };
    }
...
function createProvider(cfg: Mem0Config, api: OpenClawPluginApi): Mem0Provider {
  ...
  if (cfg.mode === "external-api") {
    return new ExternalAPIProvider({
      searchUrl: cfg.externalApi!.searchUrl,
      addUrl: cfg.externalApi!.addUrl,
      apiToken: cfg.externalApi!.apiToken,
      timeout: cfg.externalApi!.timeout || 10000,
    });
  }
...
const memoryPlugin = {
  ...
  name: "Memory (Mem0)",
  description:
    "Mem0 memory backend — Mem0 platform, self-hosted open-source, or external API",
  ...
  register(api: OpenClawPluginApi) {
    ...
    api.logger.info(
      `openclaw-mem0: registered (mode: ${cfg.mode}, user: ${cfg.userId}, autoRecall: ${cfg.autoRecall}, autoCapture: ${cfg.autoCapture})`,
    );
      ...
      ({ program }) => {
        ...
        mem0
          ...
          .action(
            async (
              query: string,
              opts: { limit: string; scope: string },
            ) => {
              try {
    ...
    if (cfg.autoRecall) {
      api.on("before_agent_start", async (event, ctx) => {
        ...
        try {
          ...
          const totalCount =
            longTermResults.length + uniqueSessionResults.length;
          api.logger.info(
